...
class UUEnumSerializerTest
{
    ...
    private fun doDeSerializeString(format: UUEnumFormat, fallback: TestEnum? = null, input: String, expected: TestEnum)
    {
        val serializer = serializer(format, fallback)
    ...
    private fun doDeSerializeInt(fallback: TestEnum? = null, input: Int, expected: TestEnum)
    {
        val serializer = serializer(UUEnumFormat.Ordinal, fallback)
    ...
    inner class SerializeTests
    {
        ...
        fun `serialize null`()
        {
            val result = Json.encodeToString(serializer(UUEnumFormat.NameSnakeCase), null)
    ...
    inner class ToFromTests
    {
        ...
        @TestFactory
        fun `test nullable deserialization returns null for unknown values`(): List<DynamicTest>
        {
            val formats = UUEnumFormat.entries.toTypedArray()
            val unknownInputs =
                listOf("\"not_valid\"", "\"UNKNOWN\"", "\"does_not_exist\"", "\"123456\"", 99, 110, -55)
            ...
            return formats.flatMap { format ->
                unknownInputs.map { input ->
                    DynamicTest.dynamicTest("Format=$format, Input=$input")
                    {
                        val ser = serializer(format, null)
                        if (ser.descriptor.kind == PrimitiveKind.STRING && input is String)
                        {
                            val result = json.decodeFromString(ser, input)
                            assertNull(result, "Expected null for input $input with format $format")
                        }
                        else if (ser.descriptor.kind == PrimitiveKind.INT && input is Int)
                        {
                            val result = json.decodeFromString(ser, "$input")
        ...
        @TestFactory
        fun `test nullable serialization of null value`(): List<DynamicTest>
        {
            val formats = UUEnumFormat.entries.toTypedArray()

            return formats.map { format ->
                DynamicTest.dynamicTest("Format=$format, Value=null")
                {
                    val ser = serializer(format)
                    val encoded = json.encodeToString(ser, null)
                    assertEquals("null", encoded, "Expected 'null' for format $format")
                }
            }
        }
    }
